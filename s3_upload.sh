...
mkdir sra-cache
cat ../extract.csv | xargs -I{} sh -c ' time aws s3 cp s3://sra-pub-run-odp/sra/{}/{} --no-sign-request ./sra-cache '
cat ../extract.csv | xargs -I{} sh -c ' time fasterq-dump -e 32 --fasta-unsorted --skip-technical -t ./sra-cache ./sra-cache/{} -Z | aws s3 cp - $S3URL/queries/{}.fa --only-show-errors '
# parallel -t --jobs 2 --tag -q -a ../extract elastic-blast submit --dry-run --query $S3URL/queries/{}.fa --db $S3URL/custom_blastdb/TOBG_NP-110.fna --program blastn --num-nodes 8 --batch-len 1000000000 --results $S3URL/results/results_parallel/output_{} -- -task megablast -word_size 28 -evalue 0.00001 -max_target_seqs 10000000 -perc_identity 97 -outfmt "6 std qlen slen qcovs"  -mt_mode 1
